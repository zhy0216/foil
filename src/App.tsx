...
export default function App() {
  ...
  const persistSettings = useCallback(
    (next: Settings) => {
      setSettings(next);
      const result = writeStorageItem('local', 'foil_settings', JSON.stringify(next));
  ...
  const adoptDoc = useCallback(
    (doc: StoredDoc, persisted = true) => {
      ...
      setReadOnly(false);
      const adopted = persisted && currentResult.ok;
      setSaveState(adopted ? 'saved' : 'error');
  ...
  const handleEditShared = () => {
    ...
    const adopted = adoptDoc(created.value, true);
    refreshDocs();
    if (adopted) showToast('Saved as a local document — your edits stay on this device');
  };
  ...
  return (
    <div className="app">
      ...
      <SettingsModal
        ...
        onReset={() => persistSettings({ ...DEFAULT_SETTINGS })}
      />
